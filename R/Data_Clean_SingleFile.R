...
# OUTPUT_FILENAME <- "Warwick_with_Cases_MTP_27_11_20.csv"
...
add_historical_data <- function(data, Historical_data){
  ...
  test <- merge(data_clean, all_group_hist, by = c("Group", "ValueType", "Value.Date", "Geography", "AgeBand"), all = TRUE)
  test2 <- distinct(test)
  return(test2)
}
...
plot2 <- function(datasets, save = TRUE, summary_table_list){
  ...
  path <- gsub(" ", "_",paste0(outputNameWithoutExtension,".pdf"))
